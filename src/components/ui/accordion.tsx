  ...
  return (
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        className={cn(
          "flex flex-1 items-center justify-between gap-4 py-5 text-left font-display text-lg transition-colors hover:text-primary [&[data-state=open]>svg]:rotate-180",
          className
        )}
        {...props}
      >
...
function AccordionContent({
  ...
  return (
    <AccordionPrimitive.Content
      ...
      {...props}
    >
      <div className={cn("pb-5 text-muted", className)}>{children}</div>
    </AccordionPrimitive.Content>
  );
}
